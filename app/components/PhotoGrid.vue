   ...
   >
    <v-card>
     ...
     <div style="display: flex; justify-content: space-between">
      <a :href="image.author_url" class="gallery_author"
       ><v-card-title> {{ image.author }} </v-card-title>
      </a>
      <v-card-actions v-if="galleryMode">
       <v-icon v-if="!image.like" @click="likePhoto(image.id)">
        mdi-heart
       </v-icon>
       <v-icon v-else color="red" @click="likePhoto(image.id)">
        mdi-heart
       </v-icon>
      </v-card-actions>

      <v-card-actions v-else>
       <v-icon @click="deleteFromLike(image.id)"> mdi-delete </v-icon>
      </v-card-actions>
     </div>
    </v-card>
   </v-col>
